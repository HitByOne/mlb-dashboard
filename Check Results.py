...
for gpk, g in all_games.items():
    ...
    away_r = int(g.get("teams",{}).get("away",{}).get("score",0) or 0)
    home_r = int(g.get("teams",{}).get("home",{}).get("score",0) or 0)
...
# Build pitcher -> game_pk map from matchups CSV so we only
# fetch box scores for games our pitchers actually pitched in
matchups_df = read("matchups")
pitcher_game_map = {}  # pitcher_last_name -> game_pk
if not matchups_df.empty:
    for _, m in matchups_df.iterrows():
        for side in ["away", "home"]:
            pit_name = str(m.get(f"{side}_pitcher", "")).strip()
            pit_team = str(m.get(f"{side}_team", "")).strip().lower()
            if pit_name and pit_name != "TBD":
                last = pit_name.split()[-1].lower()
                full = pit_name.lower()
                # Find game_pk from team_games
                gpk = None
                for tkey, tdata in team_games.items():
                    if tkey in pit_team or pit_team in tkey:
                        gpk = tdata["gpk"]
                        break
                if gpk:
                    pitcher_game_map[last] = gpk
                    pitcher_game_map[full] = gpk

# Cache box scores — only fetch when needed
box_cache = {}
...
for idx in pending.index:
    ...
    # ── K Prop ───────────────────────────────────────────
        ...
        # Look up pitcher's game from matchups first (fast), then fall back to all games
        pit_last = pitcher_name.split()[-1].lower()
        pit_full = pitcher_name.lower()
        priority_gpks = []
        if pit_last in pitcher_game_map:
            priority_gpks.append(pitcher_game_map[pit_last])
        if pit_full in pitcher_game_map and pitcher_game_map[pit_full] not in priority_gpks:
            priority_gpks.append(pitcher_game_map[pit_full])
        # Add remaining games as fallback
        search_gpks = priority_gpks + [g for g in all_games if g not in priority_gpks]

        found = False
        for gpk in search_gpks:
    ...
    # ── HR Prop ──────────────────────────────────────────
        ...
        found = False
        for gpk in list(all_games.keys()):
            box = get_cached_box(gpk)
